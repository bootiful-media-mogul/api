...
class TranscriptionClient {
	...
	String transcribe(String uid, Resource resource) {
		...
		var ext = filename.substring(1 + resource.getFilename().lastIndexOf('.'));
		var path = uid + "." + ext;
		...
		Assert.notNull(replyMessage, "the reply message must be non-null");
	...
	private void upload(String bucketName, String objectKey, Resource resource) {
		log.info("writing to bucket named [{}] with object key [{}]", bucketName, objectKey);
		try (var inputStream = resource.getInputStream()) {
			...
			log.info("wrote to bucket named [{}] with object key [{}]", bucketName, objectKey);
		}
		catch (Throwable e) {
